...
const FloatingChatbot = () => {
  ...
  return <div className="fixed bottom-6 right-6 z-50">
            ...
            <CardContent className="flex-1 p-0 flex flex-col min-h-0">
              ...
              <div className="flex-1 p-4 overflow-y-auto bg-slate-900/50">
                <div className="space-y-3">
                  {messages.map(message => <div key={message.id} className={`flex items-start space-x-2 ${message.isBot ? '' : 'flex-row-reverse space-x-reverse'}`}>
                      {message.isBot && <div className="w-6 h-6 bg-gradient-to-br from-blue-600 to-slate-500 rounded-full flex items-center justify-center flex-shrink-0">
                          <Zap size={12} className="text-white" />
                        </div>}
                      <div className={`rounded-lg px-3 py-2 max-w-xs text-sm ${message.isBot ? 'bg-slate-700/70 text-gray-200' : 'bg-blue-600/80 text-white'}`}>
                        <p>{message.text}</p>
                      </div>
                      {!message.isBot && <div className="w-6 h-6 bg-gradient-to-br from-slate-600 to-slate-700 rounded-full flex items-center justify-center flex-shrink-0">
                          <span className="text-white text-xs">U</span>
                        </div>}
                    </div>)}
                  {isLoading && <div className="flex items-start space-x-2">
              ...
              <div className="p-3 border-t border-slate-700/50 bg-slate-800/30">
                <div className="flex space-x-2">
                  <Input placeholder="Type your question..." className="flex-1 bg-slate-700/50 border-slate-600 text-white placeholder:text-gray-400 focus:border-blue-600 text-sm h-8" value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={handleKeyPress} disabled={isLoading} />
                  <Button size="sm" className="bg-gradient-to-r from-blue-600 to-slate-500 hover:from-blue-500 hover:to-slate-400 text-white h-8 w-8 p-0" onClick={sendMessage} disabled={isLoading || !inputValue.trim()}>
                    <Send size={14} />
                  </Button>
                </div>
              </div>
      ...
      {/* Minimized state */}
      {isOpen && isMinimized && <div className="mb-4">
          <Button className="bg-gradient-to-r from-blue-600/90 to-slate-500/90 hover:from-blue-600 hover:to-slate-500 text-white w-12 h-8 rounded-lg shadow-lg animate-scale-in" onClick={maximizeChat}>
            <MessageCircle size={16} />
          </Button>
        </div>}

      {/* Toggle Button */}
      <Button className={`bg-gradient-to-r from-blue-600 to-slate-500 hover:from-blue-500 hover:to-slate-400 text-white w-14 h-14 rounded-full shadow-lg hover:shadow-xl transform transition-all duration-300 hover:scale-110 ${isOpen ? 'rotate-180' : 'rotate-0'}`} onClick={toggleChat}>
